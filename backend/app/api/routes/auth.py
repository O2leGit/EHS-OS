...
async def login(req: LoginRequest, db=Depends(get_db)):
    user = await db.fetchrow(
        "SELECT id, tenant_id, password_hash, role FROM users WHERE email = $1", req.email
    )
    if not user or not verify_password(req.password, user["password_hash"]):
        raise HTTPException(status_code=401, detail="Invalid credentials")

    token = create_token(str(user["id"]), str(user["tenant_id"]), user["role"])
    return {"token": token, "user_id": str(user["id"])}
...
async def me(current_user: dict = Depends(get_current_user), db=Depends(get_db)):
    user = await db.fetchrow(
        "SELECT id, email, full_name, role, tenant_id FROM users WHERE id = $1::uuid",
        current_user["sub"],
    )
    if not user:
